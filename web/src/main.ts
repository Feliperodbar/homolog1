...
import * as storage from './core/modules/storage';
import * as ui from './core/modules/ui';
import * as capture from './core/modules/capture';
import * as stepsModule from './core/modules/steps';
import * as ocr from './core/modules/ocr';
import * as exp from './core/modules/export';
import * as backend from './core/modules/backend';
...
function setupEventListeners() {
  ...
  if (downloadHtmlBtn)
    downloadHtmlBtn.addEventListener('click', () => exp.downloadHtml(steps, projectData));
    ...
    stepsList.addEventListener('scroll', () => {
      ...
      if (nextStepBtn)
        nextStepBtn.disabled =
          !hasScroll || stepsList.scrollLeft + stepsList.clientWidth >= stepsList.scrollWidth - 10;
    });
  }
...
async function addStepWithHighlight(coords) {
  ...
  try {
    ...
    if (label) {
      ...
      steps = stepsModule.updateStep(
        steps,
        lastStep.id,
        'description',
        buildDefaultDescription(label, coords),
      );
      renderSteps();
      storage.persist(steps);
      ...
      steps = stepsModule.updateStep(
        ...
        lastStep.id,
        'description',
        buildDefaultDescription(null, coords),
      );
      storage.persist(steps);

...
describe('IndexedDB do painel',()=>{
 it('cria e recupera projeto, sessão, passo e imagem',async()=>{const r=records();await saveProject(r.project);await saveSession(r.session);await saveStep(r.step,new Blob(['image'],{type:'image/png'}));expect(await listProjects()).toHaveLength(1);expect(await listSessions(r.project.id)).toHaveLength(1);expect(await listSteps(r.session.id)).toHaveLength(1);expect(await getScreenshot(r.step.screenshotId)).not.toBeNull();});
 it('atualiza e ordena passos',async()=>{const r=records();await saveProject(r.project);await saveSession(r.session);await saveStep({...r.step,sequence:2});await saveStep({...r.step,id:newId(),screenshotId:null,sequence:1});expect((await listSteps(r.session.id)).map(s=>s.sequence)).toEqual([1,2]);});
 it('exclui projeto em cascata',async()=>{const r=records();await saveProject(r.project);await saveSession(r.session);await saveStep(r.step,new Blob(['x']));await deleteProjectCascade(r.project.id);expect(await listProjects()).toHaveLength(0);expect(await listSessions(r.project.id)).toHaveLength(0);expect(await listSteps(r.session.id)).toHaveLength(0);});
 it('sincroniza projeto, sessão, passo e screenshot da extensão',async()=>{
  const now=Date.now();
